...
from .constants import PLAZA_BASE
...
def find_show_url(show_name, show_date):
    ...
    assert isinstance(
        show_date, (str, pd.Timestamp, pd.DatetimeTZDtype, np.datetime64)
    ), "show_date must be a string or pandas Timestamp."
    ...
    show_year, show_month, show_day = (
        pd.to_datetime(show_date).year,
        pd.to_datetime(show_date).month,
        pd.to_datetime(show_date).day,
    )
    print_debug(
        f"Searching for show '{show_name}' on date {show_year}-{show_month:02d}-{show_day:02d}"
    )
    ...
    print_debug(
        f"Fetching Agility Plaza page for year {show_year} from URL: {search_URL}"
    )
    ...
    if not found_month:
        raise ValueError(
            f"Month '{target_month}' not found on Agility Plaza for year {show_year}."
        )
    ...
    for row in show_rows:
        ...
        if len(columns) >= 2:
            ...
            if (
                (target_norm in row_norm)
                or (row_norm in target_norm)
                or is_close_match(show_name, row_name)
            ):
    ...
    raise ValueError(
        f"Show name '{show_name}' not found in the month section on Agility Plaza."
    )
        ...
        if header_text == "running orders":
            ...
            if card:
                for span in card.find_all("span"):
                    ...
                    if (
                        "championship" in text.lower()
                        and height.lower() in text.lower()
                    ):
                        ...
                        if link_tag and link_tag.has_attr("href"):
                            ...
                            raise ValueError(
                                f"No link found for running orders of {text}"
                            )
                        if "Jumping" in text:
                            jumping_class.class_number = class_number
                            jumping_class.class_type = class_type
                            jumping_class.running_orders_url = PLAZA_BASE + link

                        elif "Agility" in text:
                            agility_class.class_number = class_number
                            agility_class.class_type = class_type
                            agility_class.running_orders_url = PLAZA_BASE + link
                        found_classes += 1
                        print_debug2(
                            f"Class number: {class_number}, Class type: {class_type}, link: {link}"
                        )
            ...
            for a in card_block.find_all("a"):
                ...
                if "Championship Jumping" in text and height.lower() in text.lower():
                    name = text
                    link = PLAZA_BASE + a.get("href")

                    # Store information in the class object
                    jumping_class.class_number, _, _, jumping_class.class_type = (
                        name.split(" ")
                    )
                    ...
                    found_classes += 1
                    print_debug2(
                        f"Found Championship Jumping class: {name}, link: {link}"
                    )
                    ...
                    # Store information in the class object
                    agility_class.class_number, _, _, agility_class.class_type = (
                        name.split(" ")
                    )
                    ...
                    found_classes += 1
                    print_debug2(
                        f"Found Championship Agility class: {name}, link: {link}"
                    )

    if found_classes == 0:
        raise ValueError(
            f"No championship class found for height '{height.capitalize()}'."
        )
...
def extract_class_id(class_link):
    ...
    ID = parts[4]
    assert (
        ID.isdigit() and len(ID) == 10
    ), "Extracted ID is not a valid 10-digit numeric string."
...
if __name__ == "__main__":
    ...
    try:
        matched_show, matched_showDate = check_show_in_closest(
            test_show_name, closest_shows_df
        )
