...
def process_eliminations(eliminations_text):
    ...
    eliminations = []
    eliminations_text = (
        eliminations_text[len("eliminations:") :].strip()
        if eliminations_text.lower().startswith("eliminations:")
        else eliminations_text
    )
    eliminations_text = eliminations_text.replace("&amp;", "&")
    eliminations_text = eliminations_text.strip()
...
def process_class_df(df):
    ...
    df["Place (mobile)"] = (
        df["Place (mobile)"].astype(str).str.extract("(\d+)").astype(int)
    )
...
def import_results(show_class, simulation=False):
    ...
    if show_class.results_url is None:
        raise ValueError(
            f"No results URL provided from {show_class.class_type} - returning None, None. Class status: {show_class.status}"
        )
    ...
    if not simulation:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to fetch results from {show_class.results_url}: {e}"
            )
    else:
        ...
        if show_class.class_type.lower() == "agility":
            agility_url = os.path.join(
                "NorthDerbySaves", "NorthDerbyShow_LgeAg_compeleted.html"
            )
            try:
            ...
            jumping_url = os.path.join(
                "NorthDerbySaves", "NorthDerbyShow_LgeJmp_incomplete.html"
            )
            try:
            ...
            raise ValueError(
                f"Unsupported class type for simulation: '{show_class.class_type}'. "
    ...
    if not table:
        raise ValueError(
            "No HTML table found in the results page. "
    ...
    if len(rows) < 2:
        raise ValueError(
            f"Table has insufficient rows ({len(rows)}). "
    ...
    if len(table_data) < 2:
        raise ValueError(
            f"Insufficient data rows found ({len(table_data)}). "
    ...
    if not headers:
        raise ValueError(
            "No table headers (th elements) found. Cannot determine column structure."
        )

    header_row = [header.get_text().strip() for header in headers]
    if not header_row or not any(header_row):
        raise ValueError(
            "All table headers are empty. Cannot create meaningful DataFrame columns."
        )
    ...
    for i, row in enumerate(data_rows):
        if len(row) != expected_columns:
            print_debug3(
                f"Warning: Row {i} has {len(row)} columns, expected {expected_columns}"
            )
    ...
    if df.empty:
        raise ValueError(
            "Resulting DataFrame is empty - no valid competition data found"
        )
    ...
    print_debug3(
        f"Eliminations array ({len(eliminations)} entries): {eliminations[:3] if len(eliminations) >= 3 else eliminations}"
    )
...
def import_running_orders(show_class, simulation=False):
    ...
    if show_class.running_orders_url is None:
        print_debug3(
            f"No running order URL provided from {show_class.class_type} - returning None. Class status: {show_class.status}"
        )
        return None, None
    ...
    if not simulation:
        ...
        try:
            ...
            raise requests.RequestException(
                f"Failed to fetch results from {show_class.running_orders_url}: {e}"
            )
    else:
        # Load results from local simulation files
        print_debug3(f"Loading simulation data for class type: {show_class.class_type}")
        ...
        if show_class.class_type.lower() == "agility":
            ...
            try:
                simulation_soup = read_from_file(
                    "NorthDerbySaves/NorthDerbyRunningOrders_LgeJmp.html"
                )
            except FileNotFoundError:
                raise FileNotFoundError(
                    "Simulation file 'NorthDerbySaves/NorthDerbyRunningOrders_LgeJmp.html' not found"
                )
        else:
            raise ValueError(
                f"Unsupported class type for simulation: '{show_class.class_type}'. "
    ...
    if "Name" in df.columns:
        df["Withdrawn"] = df["Name"].apply(
            lambda x: "Yes" if "Withdrawn" in x else "No"
        )
...
if __name__ == "__main__":
    from .KC_ShowProcesser import (
        find_closest_shows,
        check_show_in_closest,
        is_close_match,
    )
    ...
    print(
        f"({__name__}) From `plaza_resultsRunningOrder.py` \n({__name__}) Running import_results and import_running_orders tests..."
    )

    print("\n==== Testing Results Importer (simulation save)====")
    # Load simulation data
    simulation_soup = read_from_file(
        os.path.join("NorthDerbySaves", "NorthDerbyShow_SecondClass.html")
    )
    agility_class, jumping_class, _ = find_champ_classes(simulation_soup, "Lge")
    jumping_class_results, jumping_class_eliminations, _ = import_results(
        jumping_class, simulation=True
    )
    jumping_running_orders = import_running_orders(jumping_class, simulation=True)

    # Check the types of outputs
    assert isinstance(
        jumping_class_results, pd.DataFrame
    ), "Results should be a DataFrame"
    assert isinstance(
        jumping_running_orders, pd.DataFrame
    ), "Running orders should be a DataFrame"
    assert isinstance(
        agility_class, ClassInfo
    ), "Agility class should be a ClassInfo instance"
    assert isinstance(
        jumping_class, ClassInfo
    ), "Jumping class should be a ClassInfo instance"
    assert isinstance(jumping_class_eliminations, list), "Eliminations should be a list"

    # Print summaries
    print(
        f"({__name__})",
        f"Jumping Results DataFrame ({type(jumping_class_results)}):\n",
        jumping_class_results.head(),
    )
    print(
        f"\n({__name__})",
        f"Jumping Running Orders DataFrame ({type(jumping_running_orders)}):\n",
